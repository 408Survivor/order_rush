...
> 最后更新：2026-09-04（Session 19：**#83/#93/#103 三连收官**（PR #101/#94/#104 全合并，窗口店模式落地）+ **《杯杯倒满》整包解包逆向**（工程在 `_analysis/feed_the_cups/`，对标路线挂 #95-#100）；**#105 设备桌改长条桌待修**；冒烟 **412 断言**）
...
| — | 💓❤️ 压力与成长线 | ✅ **完成** | #82 心率值（PR #88）+ #83 店铺星级五档成长（PR #101）均合并 |
| — | 🏪 窗口店改造 | ✅ **完成** | #93 布局大改+设备可搬上桌+顾客寻路（PR #94）→ #103 窗口店模式+点单/取餐分离（PR #104）：店内纯操作区、顾客全程店外、柜台单向流水线 |
...
| #83 | [玩法] 繁荣度/店铺星级：长线成长目标（对标烘焙糕手） | ✅ 已完成（PR #101） |
...
| #93 | [玩法] 布局大改：设备可拿起摆上桌 + 顾客寻路（对标烘焙糕手） | ✅ 已完成（PR #94） |
| #103 | [玩法] 窗口店模式 + 点单/取餐流水线分离（对标杯杯倒满） | ✅ 已完成（PR #104） |
| #105 | [观感] 设备桌修复：两张斜短桌 → 一张长条工作台 | **待办（开发者实测反馈，下会话优先）** |
| #95 | [玩法] 天数序列关卡编排（对标杯杯倒满） | 待办（对标主线） |
| #96 | [玩法] 契约系统：选难获益 debuff 三选一（对标杯杯倒满） | 待办（对标主线） |
| #97 | [玩法] 主题日事件包：特殊事件 2 → 10+ | 待办 |
| #98 | [数值] 心率 STRESS_* 常量调平（对照杯杯倒满压力公式） | 待办（需开发者 F5 长局） |
| #99 | [玩法] 多角色扩展：角色扩容 + 特质差异 + 独立熟练度 | 待办（P8 深化） |
| #100 | [玩法] 局外货币与永久加成（元进度） | 待办 |
| #102 | [BUG 排查] 结算面板引擎侧幻影点击自动跳过 | 待办（低优先级，疑似自动化环境特有） |
| — | [P7 二期] L2 炒锅 / L3 现做流程（2 L2 + 1 L3） | 待办（可开 issue；解锁条件拟衔接 #83 星级） |
| — | [运行实测] 窗口店手感 + #82/#83 数值长局平衡 | 待办（开发者 F5） |
...
**2026-09-04 Session 19（#83 收尾 + #93/#103 合并 + 《杯杯倒满》解包逆向与对标路线）**

| 事件 | 详情 |
|------|------|
| ✅ #83 店铺星级收官（PR #101） | 补结算面板截图验收（星级行+繁荣度进度条渲染正常）→ PR 合并、issue 自动关闭；冒烟基线 334→**351** |
| ⚠️ 发现幻影点击疑似 bug → issue #102 | 运行模式打烊瞬间 `NextDayButton.pressed` 被引擎侧触发（print_stack 无 GDScript 来源），结算面板一闪而过；自动化截图环境特有嫌疑大（F5 实测正常），低优先级备查 |
| ✅ PR #94（#93 布局大改）rebase + 合并 | #83 先合并后 rebase 唯一冲突在 `upgrade_manager.gd` save_upgrades（#83 改读-改-写 vs #94 整写）→ 保留读-改-写骨架追加 device_positions，两存档共存；冒烟 **395** 两轮 PASS；开发者 F5 实测通过后合并 |
| 🔓 《杯杯倒满》整包解包 | 工程：`/Users/liutongqing/Downloads/GameDev/_analysis/feed_the_cups/`（仓库外）。工具 GDRE Tools v2.6.4（`Godot RE Tools.app`，CLI 用 `--headless --recover/--extract/--list-files`）。**密钥提取**：二进制带调试符号 → `nm` 找到 `_script_encryption_key`（0x101faaec0）→ `otool -l/-f` 段映射换算文件偏移读 32 字节。**配置 cfg 解密**：`save_encrypted_pass` GDEC 格式，AES-256-**ECB** 无 IV，key = md5("Cups") 的 hex ASCII 32 字节。356 个 .gde 全部反编译为可读 .gd；场景/配置全可读 |
| 📋 杯杯倒满系统盘点 | 22 个系统全列（日循环天数序列/订单四层时间轴/380 配方/333 卡+150 契约+74 事件/58 设备/压力/128 顾客/员工抽卡/AStar 五张寻路网/家园/装扮/联机等）；配置表规模与"值得借鉴 12 点"已整理（记录在会话，要点进了 #95-#100 issue） |
| 🎯 对标路线拍板 | **不抄**：联机、家园场景、装扮、自主员工 AI；**澄清**：①他们"角色"=玩家操作的可玩角色（每次选一个，特质各异，熟练度独立）→ 对应 #99 深化 P8；②家园内核=局内结算按倍率转局外货币→解锁永久加成 → 对应 #100 元进度。**反面教材**：他们 333 张卡效果硬编码散落巨型脚本（NPC_Rigid.gd 5887 行），我们坚持 Modifier 三查通道 |
| 🔍 杯杯倒满布局逆向 | 柜台=接口（内侧操作区/外侧客流区寻路网不相交、柜台钉死）；台面+机器两级（机器必须上台面才工作，CanLayout/Lift/拆装耗时字段）；设备靠墙 U 形+走道成环；小店=纯窗口店（顾客永不进店，柜台临街开窗）；点单台注册全局 OrderV2 目标点（换布局零配置）；100px 网格 + AStar2D 连线前打物理射线检测连通性 |
| ✅ #103 窗口店模式（PR #104） | 店内 2304×1296→**2304×1040**（南带 108→364 成人行道）；吧台南移临街 (1152,990)；**点单台 (700,960) / 取餐台 (1500,960) / 外卖窗口 (1930,960)** 三点流水线；餐桌椅/就餐区移除；顾客全程店外：点单队列左延（上限 3=生成闸门）→ 下单转取餐队列右延；店外 L 形导航；玩家交付从"E 对顾客"改为"E 对取餐台按菜品匹配订单"；顾客从 interactable 组移除；冒烟 **412** 两轮 PASS |
| 💡 y-sort 按子树根排序 | 取餐台整棵子树被吧台盖住（子树内部 z 无效，按根的 y 排）→ 根 sprite z=1 修复。新道具被吧台/大家具盖住先查这个 |
| 💡 thorvg 不渲染 SVG `<text>` | Godot 的 SVG 渲染器（thorvg）不支持 `<text>` 元素——SVG 里的文字一律转路径/图标，招牌文字改用图形 |
| 💡 双队列时序断言按槽位实测 | 点单→取餐行走距离按槽位序号递增（800/1000/1200px），时序断言必须先实测路径长度再写数，拍脑袋写会成片 FAIL |
| 📌 冒烟断言数 | **412**（#83 后 351 → #93 后 395 → #103 后 412） |
| 📌 环境注意 | 子代理又遇一次 5 小时配额 403 中断（#103 开工即断），resume 后续跑完成；长任务留意配额窗口 |
